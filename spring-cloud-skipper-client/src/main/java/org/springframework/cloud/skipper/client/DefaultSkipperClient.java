...
public class DefaultSkipperClient implements SkipperClient {
	...
	@Override
	public List<Release> getReleases(String releaseNameLike) {
		...
		if (StringUtils.hasText(releaseNameLike)) {
			url = String.format("%s/%s/%s/%s", baseUrl, "releases", "listByName", releaseNameLike);
		}
		else {
			url = String.format("%s/%s/%s", baseUrl, "releases", "list");
		}
		return this.restTemplate.exchange(url, HttpMethod.GET, null, typeReference, new HashMap<>()).getBody();
	}

	@Override
	public List<Release> getRevisions(String releaseName, String maxRevisions) {
		ParameterizedTypeReference<List<Release>> typeReference = new ParameterizedTypeReference<List<Release>>() {
		};
		Map<String, Object> parameters = new HashMap<>();
		String url = String.format("%s/%s/%s/%s/%s", baseUrl, "releases", "revisions", releaseName, maxRevisions);
		return this.restTemplate.exchange(url, HttpMethod.GET, null, typeReference, parameters).getBody();
	}

	@Override
	public Resources<Release> getRevisions(String releaseName) {
